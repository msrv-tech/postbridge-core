...
async def _send_start_view(message: Message, *, backend: BotBackend, i18n) -> None:
    user = message.from_user
    if not user:
        return
    locale = _resolve_locale(i18n)
...
async def _handle_cmd_start(message: Message, *, backend: BotBackend, i18n) -> None:
            ...
            if arg0.startswith("web_"):
                ...
                if not data.get("ok"):
                    ...
                    if reason == "invalid_or_expired":
                        ...
                        await message.answer(i18n.translate("bot.auth.failed_generic", locale=locale))
                    return
                await _send_web_handoff(
                    message,
                    backend=backend,
                ...
                return
    await _send_start_view(message, backend=backend, i18n=i18n)


async def _handle_my_chat_member(update: ChatMemberUpdated, *, backend: BotBackend, i18n) -> None:
    chat = update.chat
...
async def _handle_cmd_open_web(message: Message, *, backend: BotBackend, i18n) -> None:
    workspace_id = None
    locale = _resolve_locale(i18n)
...
async def _handle_text_fallback(message: Message, *, backend: BotBackend, i18n) -> None:
    workspace_id = None
    locale = _resolve_locale(i18n)
    if message.from_user:
        workspace_id = await backend.resolve_workspace_id(
...
async def _handle_message_fallback(message: Message, *, backend: BotBackend, i18n) -> None:
    workspace_id = None
    locale = _resolve_locale(i18n)
    if message.from_user:
        workspace_id = await backend.resolve_workspace_id(
...
async def _handle_channel_post(message: Message, *, backend: BotBackend) -> None:
...
async def _handle_edited_channel_post(message: Message, *, backend: BotBackend) -> None:
    chat_id = message.chat.id if message.chat else None
    if chat_id is None:
        return
    target_data = await backend.live_sync_target(chat_id)
...
async def _handle_unknown_callback_fallback(callback: CallbackQuery, *, backend: BotBackend, i18n) -> None:
    workspace_id = None
    locale = _resolve_locale(i18n)
...
def build_router(backend: BotBackend) -> Router:
    ...
    async def cmd_start(message: Message) -> None:
        await _handle_cmd_start(message, backend=backend, i18n=i18n)

    @router.my_chat_member(F.chat.type.in_({"channel", "supergroup"}))
    async def on_my_chat_member(update: ChatMemberUpdated) -> None:
        await _handle_my_chat_member(update, backend=backend, i18n=i18n)

    @router.message(
        or_f(Command("add"), Command("sync"), Command("plan"), Command("max"), Command("vk"), Command("rss"))
    )
    async def cmd_open_web(message: Message) -> None:
        await _handle_cmd_open_web(message, backend=backend, i18n=i18n)

    @router.message(F.text & ~F.text.startswith("/"))
    async def on_text_fallback(message: Message) -> None:
        await _handle_text_fallback(message, backend=backend, i18n=i18n)

    @router.message()
    async def on_message_fallback(message: Message) -> None:
        await _handle_message_fallback(message, backend=backend, i18n=i18n)

    @router.channel_post()
    async def on_channel_post(message: Message) -> None:
        await _handle_channel_post(message, backend=backend)

    @router.edited_channel_post()
    async def on_edited_channel_post(message: Message) -> None:
        await _handle_edited_channel_post(message, backend=backend)

    @router.callback_query()
    async def on_unknown_callback_fallback(callback: CallbackQuery) -> None:
        await _handle_unknown_callback_fallback(callback, backend=backend, i18n=i18n)
